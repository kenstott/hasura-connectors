...
const start = async () => {
    try {
        const databaseNames = fs.readdirSync(process.env.CSV_CONNECTOR_FILES || 'src/data/databases');
        staticData = {};
        for (let i = 0; i < databaseNames.length; i++) {
            const dbPath = path.resolve(process.env.CSV_CONNECTOR_FILES || 'src/data/databases', databaseNames[i])
            staticData[`$${databaseNames[i]}`] = await loadStaticData(dbPath);
        }
...
start().then(() => server.log.info("Started"));
